...
Schedule::Schedule(const char *name)
    : name(name), state(ScheduleState::OFF), nextStartSec(0), activeStartTime(0)
{
}

void Schedule::setTiming(uint32_t startSec, uint32_t durationSec, uint32_t intervalSec)
{
...
void Schedule::triggerEnd()
{
    ...
    if (t.intervalSec > 0)
    {
        nextStartSec += t.intervalSec;

        // Handle midnight rollover
        if (nextStartSec >= 86400UL)
        {
            nextStartSec %= 86400UL;
        }
    }
    else
    {
...
void Schedule::update(uint32_t currentTimeSec)
{
    ...
    else if (state == ScheduleState::ACTIVE)
    {
        ...
        if (endTime >= 86400UL)
        {
            ...
            shouldEnd = (currentTimeSec >= wrappedEnd && activeStartTime > currentTimeSec);
        }
        else
        {
...
uint32_t Scheduler::getSecondsSinceMidnight()
{
    ...
    return timeinfo.tm_hour * 3600UL +
